...
// Assign indexes and remaining lengths to zero
// Apply Tx/Rx events
...
/**
 * -> Transmit tx data using twi_writeTo()
 * -> reset tx buffer index and length
 * -> Lower transmitting flag
 * -> return qty of data read
 */
...
/**
 * -> If transmitting flag is high
 * --> setWriteError then return 0 if the buffer is full
 * --> Put data into tx buffer at index. Add one to index.
 * --> Update buffer length
 * -> If tranmitting flag was not high, just transmit the data byte pointer
 */
...
/** Original:
 * -> If transmitting
 * --> send all data to write(uint8_t)
 * -> Else directly transmit data array pointer
 *
 * Could be optimised by simply doing write(uint8_t) operations here faster.
 */
...
/**Original:
 * -> Return single byte from the buffer
 * -> increement rx buffer index
 * -> return -1 if failed
 *
 * This will likely just be reading data directly from I2C node
 */
...
/**
 * -> Get data from the buffer without incrementing the buffer index so that
 *    the data can be read in later.
 *
 * Without implementing immediate capture of data, this will at best be sketchy.
 */
...
/**
 * Original did not implement this.
 */
